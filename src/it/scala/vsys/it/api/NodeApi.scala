...
import vsys.blockchain.state.Portfolio
...
import vsys.api.http.payment.{PaymentRequest, SignedPaymentRequest}
import vsys.utils.{LoggerFacade, ScorexLogging}
import vsys.api.http.debug.DebugApiRoute.portfolioFormat
import vsys.api.http.debug.RollbackParams
...
trait NodeApi {
  ...
  def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long, feeScale: Short): Future[Transaction] =
    postJson("/vsys/payment", PaymentRequest(amount, fee, feeScale, sourceAddress, None, recipient)).as[Transaction]

  def signedPayment(transfer: SignedPaymentRequest): Future[Transaction] =
    postJson("/vsys/broadcast/payment", transfer).as[Transaction]
...
object NodeApi extends ScorexLogging {
  ...
  case class Transaction(`type`: Int, id: String, feeCharged: Long, timestamp: Long)
