...
class ReissueTransactionSpecification(override val allNodes: Seq[Node], override val notMiner: Node)
  ...
  test("asset reissue changes issuer's asset balance; issuer's vsys balance is decreased by fee") {
    val f = for {
      _ <- assertBalances(firstAddress, 100.vsys, 100.vsys)

      issuedAssetId <- sender.issue(firstAddress, "name", "description", defaultQuantity, 2, reissuable = true, fee = 10.vsys).map(_.id)
      ...
      reissuedAssetId <- sender.reissue(firstAddress, issuedAssetId, defaultQuantity, reissuable = true, fee = 10.vsys).map(_.id)
