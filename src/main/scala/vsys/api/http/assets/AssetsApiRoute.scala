...
import vsys.api.http.{ApiError, ApiRoute, InvalidAddress, BroadcastRoute}
...
import vsys.blockchain.transaction.{AssetAcc, AssetIdStringLength, TransactionFactory}
...
case class AssetsApiRoute(settings: RestAPISettings, wallet: Wallet, utx: UtxPool, allChannels: ChannelGroup, state: StateReader, time: Time)
    ...
    (get & path(Segment / "distribution")) { assetId =>
      complete {
        Success(assetId).filter(_.length <= AssetIdStringLength).flatMap(Base58.decode) match {
          ...
          case Failure(_) => ApiError.fromValidationError(vsys.blockchain.transaction.ValidationError.GenericError("Must be base58-encoded assetId"))
        }
      }
    }
  ...
  @ApiImplicitParams(Array(
    new ApiImplicitParam(
      ...
      required = true,
      paramType = "body",
      dataType = "vsys.api.http.assets.TransferRequest",
  ...
  @ApiImplicitParams(Array(
    new ApiImplicitParam(
      ...
      required = true,
      paramType = "body",
      dataType = "vsys.api.http.assets.IssueRequest",
  ...
  @ApiImplicitParams(Array(
    new ApiImplicitParam(
      ...
      required = true,
      paramType = "body",
      dataType = "vsys.api.http.assets.ReissueRequest",
  ...
  @ApiImplicitParams(Array(
    new ApiImplicitParam(
      ...
      required = true,
      paramType = "body",
      dataType = "vsys.api.http.assets.BurnRequest",
