...
object WebhookSettings {
	...
	private def setEventSettings(config: Config): Either[ValidationError, WebhookEventSettings] = {
		val path = "rules"
		val afterH = config.as[Long](s"$path.afterHeight")
		val afterT = config.as[Long](s"$path.afterTime")

		config.as[String]("type") match {

			case "1" | "Block Appended" => 
				Right(BlockAppendedEventSettings(
					config.as[Boolean](s"$path.withTxs"), 
					afterH, afterT))

			case "2" | "Tx Confirmed" =>
				Right(TxConfirmedEventSettings(
					config.as[Seq[String]](s"$path.relatedAccount"),
			 	   	afterH, afterT,
			...
			case "3" | "State Updated" =>
				Right(StateUpdatedEventSettings(
					config.as[Seq[String]](s"$path.relatedAccount"), 
					afterH, afterT))

			case "4" | "Block Rollback" => 
				Right(BlockRollbackEventSettings(
					config.as[Seq[String]](s"$path.relatedAccount"),
				   	afterH,afterT,
