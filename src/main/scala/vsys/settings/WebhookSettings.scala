	...
	private def setEventSettings(config: Config): Either[ValidationError, WebhookEventSettings] = {
		...
		config.as[String]("type") match {
			...
			case "2" | "Tx Confirmed" =>
				...
				Right(TxConfirmedEventSettings(acc, afterH, afterT, inTypes,
				   							   exTypes, gteVal, gtVal, lteVal,
											   ltVal, withFeeVal))
			...
			case "4" | "Block Rollback" => 
				...
				Right(BlockRollbackEventSettings(acc, afterH, afterT,
											     wTxsOfTypes, wTxsOfAccs, wStateOfAccs))
			case _ => 
