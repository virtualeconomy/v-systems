...
object StorageFactory {
  ...
  def apply(db: DB, settings: BlockchainSettings, eventSettings: EventSettings ,renew: Boolean = false): (History, StateWriter, StateReader, BlockchainUpdater) = {
    val lock = new RWL(true)
    // TO DO
    // create actor based on url in eventSettings
    ...
    val bcu = BlockchainUpdaterImpl(stateWriter, historyWriter, settings.functionalitySettings, eventSettings.webhookSettings, settings.minimumInMemoryDiffSize, lock)
