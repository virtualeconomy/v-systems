...
case class DbPutTransaction private(sender: PublicKeyAccount,
  ...
  lazy val toSign: Array[Byte] = Bytes.concat(
    ...
    BytesSerializable.arrayWithSize(dbEntry.bytes.arr),
    Longs.toByteArray(fee),
    Longs.toByteArray(timestamp))

  override lazy val json: JsObject = jsonBase() ++ Json.obj(
    "dbKey" -> dbKey,
    "dbEntry" -> dbEntry.json,
    "fee" -> fee,
    "timestamp" -> timestamp
  )

  lazy val storageKey: ByteStr = DbPutTransaction.generateKey(sender.toAddress, dbKey)
    //ByteStr(sender.toAddress.bytes.arr ++ dbKey.getBytes("UTF-8"));
...
object DbPutTransaction {
  ...
  def parseTail(bytes: Array[Byte]): Try[DbPutTransaction] = Try {
    ...
    val (dbKeyBytes, dbKeyEnd) = Deser.parseArraySize(bytes, KeyLength)
    ...
    (for {
      ...
      tx <- DbPutTransaction.create(sender, new String(dbKeyBytes, "UTF-8"), dbEntry, fee, timestamp, signature)
  ...
  def create(sender: PrivateKeyAccount,
    ...
    create(sender, dbKey, dbEntry, fee, timestamp, ByteStr.empty).right.map { unsigned =>
