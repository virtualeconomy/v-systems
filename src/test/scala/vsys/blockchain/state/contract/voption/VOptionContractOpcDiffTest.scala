...
import vsys.blockchain.contract.ContractVOption._
...
import vsys.blockchain.contract.voption.{VOptionContractGen, VOptionFunction}
...
class VOptionContractOpcDiffTest extends PropSpec
  with PropertyChecks
  with GeneratorDrivenPropertyChecks
  with Matchers
  with TransactionGen
  with VOptionContractGen
  with VOptionFunction {
  ...
  val regWrongOpcFunContract: Gen[Contract] =
    Gen.const(Contract.buildContract(Deser.serilizeString("vdds"), Ints.toByteArray(2),
      wrongOpcTrigger,
      Seq(supersedeFunc, activateFunc, mintFunc, unlockFunc, executeFunc, collectFunc),
  ...
  val preconditionsAndRegContractWrongFun: Gen[(RegisterContractTransaction, RegisterContractTransaction, RegisterContractTransaction)] = for {
    ...
    data: Seq[DataEntry] <- initVOptionDataStackGen(tokenId.arr, tokenId.arr, tokenId.arr, tokenId.arr, ts+100, ts+200)
    description <- validDescStringGen
    validCreate <- registerVOptionGen(master, validContract, data, description, fee, ts + 1)
    wrongOpcCreate <- registerVOptionGen(master, wrongOpcContract, data, description, fee + 10000000000L, ts+2)
    wrongDataCreate <- registerVOptionGen(master, wrongDataContract, data, description, fee + 10000000000L, ts+3)
